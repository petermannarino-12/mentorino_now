...
const SettingsPage: React.FC<SettingsPageProps> = ({ onLogout, currentUser }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setNotification('Failed to save (demo mode)');
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'security':
        return (
          <div className="bg-white p-6 sm:p-10 md:p-12 rounded-[32px] sm:rounded-[48px] border border-black/[0.03] shadow-sm">
            <h3 className="text-lg sm:text-xl font-black uppercase mb-6 sm:mb-10">Security Settings</h3>
            <p className="text-xs text-slate-500 leading-relaxed font-medium">Manage your password, multi-factor authentication, and active sessions here.</p>
          </div>
        );
      case 'billing':
        return (
          <div className="bg-white p-6 sm:p-10 md:p-12 rounded-[32px] sm:rounded-[48px] border border-black/[0.03] shadow-sm">
            <h3 className="text-lg sm:text-xl font-black uppercase mb-6 sm:mb-10">Billing & Subscription</h3>
            <p className="text-xs text-slate-500 leading-relaxed font-medium">View your invoices, update payment methods, and manage your subscription plan.</p>
          </div>
        );
      default:
        return null;
