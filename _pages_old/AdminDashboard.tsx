...
import { Application, Booking, User, TaskActivity, NetworkEvent, Announcement, ResourceLink } from '../src/types';
...
Mentorino Support Team`;
  ...
  const analyzeApplication = async (app: Application) => {
    ...
    setTimeout(() => {
      setAppInsights(prev => ({ ...prev, [app.id]: { summary: 'Application analysis unavailable (backend removed)', score: 0, strengths: [], concerns: [], loading: false } }));
    }, 500);
  };
  ...
  const handleAiChat = async (presetMsg?: string) => {
    ...
    setIsAiLoading(true);
    setTimeout(() => {
      setChatHistory(prev => [...prev, { role: 'model', content: "AI chat is unavailable (backend removed). Please use the mock data features." }]);
  ...
  const startBriefing = async (booking: Booking) => {
    ...
    setTimeout(() => {
      setChatHistory(prev => [
        ...
        { role: 'model', content: "Session briefing is unavailable (backend removed)." }
      ]);
      setIsAiLoading(false);
    }, 500);
  };
        ...
        {upcomingBookings.length > 0 ? upcomingBookings.map(s => (
          <div key={s.id} className="bg-white p-6 rounded-[40px] border border-black/[0.03] shadow-sm space-y-4 hover:border-black/10 transition-all">
             ...
             <div className="flex gap-2">
               ...
               <button onClick={() => startBriefing(s)} className="px-5 py-4 bg-slate-50 text-slate-400 rounded-2xl hover:text-black active:scale-95 transition-all" aria-label="View AI Briefing"><Zap size={16} /></button>
             </div>
          </div>
